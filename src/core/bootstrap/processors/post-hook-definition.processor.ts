...
export class PostHookDefinitionProcessor extends BaseTableProcessor {
  ...
  async afterUpsert(record: any, isNew: boolean, context?: any): Promise<void> {
    ...
    if (!isMongoDB && record._methods && Array.isArray(record._methods)) {
      ...
      if (methodIds.length > 0) {
        const junctionTable = 'post_hook_definition_methods_method_definition';
        await this.queryBuilder.delete({
          table: junctionTable,
          where: [{ field: 'postHookDefinitionId', operator: '=', value: record.id }],
        });
        const junctionData = methodIds.map((methodId) => ({
          methodDefinitionId: methodId,
          postHookDefinitionId: record.id,
        }));
        ...
        this.logger.log(
          `   🔗 Linked ${methodIds.length} methods to post-hook ${record.name}`,
        );
      }
    }
  }
  ...
  protected getRecordIdentifier(record: any): string {
    ...
    return `[Post-Hook] ${record.name}${routeStr ? ` on ${routeStr}` : ''}${methodsStr ? ` (${methodsStr})` : ''}`;
  }
}